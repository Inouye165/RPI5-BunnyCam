...
- [x] Phase 6 — Dataset packaging and training workflow updates
- [ ] Phase 7 — Inference rollout, thresholds, observability, and tuning
- [ ] Phase 8 — Cleanup, documentation, and future follow-ons
...
#### Phase 6
- [x] Add `"bunny"` support to detection packaging
- [x] Add hard-case annotation/export path for proposal-only samples
- [x] Keep identity packaging conservative
- [x] Update training CLI and docs
- [x] Add packaging and workflow tests

#### Phase 7
- [x] Add feature flags for rollout
- [x] Add observability for capture and review outcomes
- [x] Add Pi-safe env tuning seams for validation
- [x] Write Pi operator validation playbook and confirm status surfaces locally
...
| 2026-04-06 | Phase 7 (Pi tuning continuation) | Added the remaining Pi-oriented validation support without changing the live detector path. candidate_collection.py now reads a minimal set of env-backed tuning knobs at config-construction time: `BUNNYCAM_BUNNY_HARD_CASE_CONF_MAX` for bunny hard-case sensitivity plus `BUNNYCAM_FALLBACK_COOLDOWN_SEC` and `BUNNYCAM_FALLBACK_MAX_PER_SESSION` for fallback spam control. README.md now documents which runtime counters to watch while tuning these knobs on-device, and tests cover unchanged defaults, env override plumbing, hard-case sensitivity changes limited to rabbit-alias cat paths, and fallback spam control without regressing normal detector-positive saves. Tightened fallback cooldown gating so the first fallback save is never suppressed just because a caller uses a relative synthetic timestamp during tests or validation. | Full pytest before change: 249 passed. Full pytest after change: 253 passed, 0 failed. | Actual Pi-side tuning remains deferred; this step only adds the smallest safe and reversible config seam for on-device validation. |
| 2026-04-06 | Phase 7 (Pi validation playbook) | Validated that the existing Phase 7 status surfaces are live from the current Windows workspace, but confirmed this environment is laptop-backend only and cannot perform true on-device Pi validation. Added a Pi operator playbook in `docs/bunny-phase7-pi-validation.md` that documents exact startup commands, status endpoints, key counters, knob meanings, adjustment order, observation timing, note-taking expectations, stop criteria, and rollback to defaults. README.md now points operators to that playbook. No runtime code changes were made because the current evidence did not justify threshold churn. | Full pytest before doc change: 253 passed. No post-change runtime tests required; documentation-only update. | Real Phase 7 threshold tuning is still pending on Raspberry Pi hardware with actual bunny activity. |
...
- Before this phase, reviewed hard-case items could enter detector packaging
  incidentally when they happened to have a frame and bbox. Phase 6 makes
  that decision explicit in review metadata and keeps annotation-bound items
  visible instead of silently mixing them into detector-positive data.
...
- The remaining safe Phase 7 knobs are now intentionally small: one
  sensitivity threshold for rabbit-alias hard-case capture and two fallback
  spam controls. Further tuning should happen on-device against the exposed
  counters before any broader runtime changes are considered.
- This workspace has now verified the status surfaces and documented the exact
  Pi-side operator procedure, but it still cannot supply real on-device bunny
  evidence because the current environment is Windows laptop mode rather than a
  Pi camera runtime.
